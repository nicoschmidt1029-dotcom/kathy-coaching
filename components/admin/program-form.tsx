...
export function ProgramForm({ contentKey, entry, defaults }: { contentKey?: string; entry?: CmsEntry; defaults: ProgramData }) {
  ...
  return <form action={saveProgram} className="space-y-6">
    ...
    <div className="sticky bottom-3 flex flex-wrap gap-3 rounded-2xl border border-black/10 bg-[#fbf8f2]/95 p-4 shadow-lg backdrop-blur"><button name="status" value="draft" className="rounded-xl border border-black/10 bg-white px-5 py-2.5 text-sm">Save draft</button><button name="status" value="published" className="rounded-xl bg-[var(--plum)] px-5 py-2.5 text-sm text-white">Save & publish</button></div>
  </form>;
}
