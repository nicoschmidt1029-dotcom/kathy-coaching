...
export function SaveButton({ children, value, variant = "primary" }: { children: React.ReactNode; value: string; variant?: "primary" | "secondary" }) {
  ...
  return <button name="status" value={value} disabled={pending} className={variant === "primary" ? "min-h-11 rounded-xl bg-[var(--plum)] px-5 py-2.5 text-sm text-white disabled:opacity-60" : "min-h-11 rounded-xl border border-black/10 bg-white px-5 py-2.5 text-sm disabled:opacity-60"}>{pending ? "Saving…" : children}</button>;
}
