...
type Props = { content?: { eyebrow?: string; headline?: string; body?: string; submitLabel?: string; image?: string | null } };
...
export function Contact({ content }: Props = {}) {
  ...
  return <section id="kontakt" className="section-pad section-pad-top-tight"><div className="container-page"><div className="max-w-2xl"><p className="eyebrow">{content?.eyebrow || t("eyebrow")}</p><DisplayTitle className="mt-4">{content?.headline || t("title")}</DisplayTitle>{content?.body && <p className="section-lede max-w-xl">{content.body}</p>}</div><div className="mt-10 grid items-start gap-8 md:mt-14 md:grid-cols-12 md:gap-12">{image && <div className="md:col-span-4"><div className="relative aspect-[3/4] overflow-hidden rounded-2xl"><Image src={image} alt={content?.eyebrow || t("eyebrow")} fill sizes="(max-width: 768px) 100vw, 30vw" className="object-cover object-[50%_20%]" /></div></div>}<div className="md:col-span-8">{state.status === "sent" ? <div role="status" aria-live="polite" className="card-pad flex min-h-[360px] flex-col items-start justify-center rounded-2xl card-surface"><CheckCircle2 className="size-8 text-[var(--plum)]" /><h2 className="card-title mt-6 text-2xl">{t("sentTitle")}</h2><p className="mt-3 text-foreground/70">{t("sentBody")}</p><button type="button" onClick={() => setState({ status: "idle" })} className="mt-7 text-sm text-[var(--plum)] underline">{t("sendAnother")}</button></div> : <form onSubmit={onSubmit} noValidate className="card-pad rounded-2xl card-surface"><label aria-hidden className="pointer-events-none absolute left-[-9999px]">Website<input name="website" tabIndex={-1} autoComplete="off" /></label><div className="space-y-5"><div><Label htmlFor="name" className="mb-2">{t("nameLabel")}</Label><Input id="name" name="name" required autoComplete="name" className="h-12" /></div><div><Label htmlFor="email" className="mb-2">{t("emailLabel")}</Label><Input id="email" name="email" type="email" required autoComplete="email" className="h-12" /></div><div><Label htmlFor="message" className="mb-2">{t("messageLabel")}</Label><Textarea id="message" name="message" required rows={6} className="min-h-36" /></div></div>{state.status === "error" && <div role="alert" className="mt-6 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-900">{state.message}</div>}<div className="mt-7 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-foreground/55">{t("privacyNote")}</p><Button type="submit" size="lg" disabled={state.status === "sending"} className="h-12 w-full bg-[var(--plum)] px-7 text-white sm:w-auto">{state.status === "sending" ? t("submitting") : content?.submitLabel || t("submit")}</Button></div></form>}</div></div></div></section>;
}
