...
export default async function AboutAdmin() {
  ...
  const biography = localized((m) => [m.about.storyMovement, m.about.storyFood, m.about.storyInnerHealth, m.about.storyEducation, m.about.storyDream, m.about.storyMission].map((paragraph) => paragraph.replace(/<\/?em>/g, "")).join("\n\n"));
  return <><AdminPageHeading title="About Katey" description="Update Katarina's biography, personal opening and main portrait in one place." /><div className="space-y-5"><WebsiteContentForm contentKey="about" title="Main About content" description="Edit the heading, full biography and portrait." entry={entry("about")} defaults={{ eyebrow: localized((m) => m.about.eyebrow), headline: localized((m) => m.about.overlapTitle), body: biography }} imageHint="Recommended: portrait image, at least 1200 px high." /><EditorialSectionForm contentKey="about-details" title="Personal opening" description="Edit the short personal statement shown beside the portrait." entry={entry("about-details")} preview="/en/katey" defaults={{ calling: localized((m) => m.about.calling.replace(/<\/?em>/g, "")) }} /></div></>;
}
